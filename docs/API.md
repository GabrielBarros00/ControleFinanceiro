...
| **Transações** | `/{ws}/transactions` | `GET` (filtros: mês, busca, categoria, `uncategorized`, método, tag, `merchant_id`, `settled`), `POST`, `PUT`, `DELETE` (os dois aceitam `settled`: "já foi paga", ADR 0029); `POST /preview` (dry-run da divisão); `POST /bulk`; compra parcelada: `GET/PUT/DELETE /{id}/installment-group` (editar/excluir o grupo inteiro) + `POST /{id}/installment-group/cancel` |
...
| **Estabelecimentos** | `/{ws}/merchants` | `GET` (com `transaction_count` do que você vê), `POST`, `PUT /{id}` (`aliases` substitui a lista; `default_category_id: null` tira a categoria padrão), `DELETE /{id}` (os lançamentos perdem o vínculo), `POST /{id}/merge` (`{into_id}`: este some e passa lançamentos e apelidos), `GET /spending?month=` (valor cheio e sua parte por estabelecimento no mês). Lançamento e recorrência aceitam `merchant_id`; o lançamento também `merchant_name` (acha pelo nome ou apelido, ou cria), e a lista filtra por `merchant_id` — ADR 0038 |
...
| **Recorrências (despesa)** | `/{ws}/recurring` | CRUD + geração/materialização de instâncias. `POST /{id}/preview` devolve, **sem escrever nada**, o que acontece com cada lançamento (`move`/`update`/`cancel`/`create`/`none`); o `PUT` aplica os escolhidos via `?apply_to=&create_occurrence=&since=` e o `DELETE` cancela os escolhidos via `?cancel_instance=` (ADR 0030). `?scope=none\|future\|all` segue valendo para quem chama sem revisão — e ali a data continua congelada. `end_date` (ou `end_after_occurrences`, convertido no servidor) dá fim à série. Assinatura (ADR 0039): `is_subscription`, `plan`, `trial_ends_on`, `notes`; estabelecimento por `merchant_id` ou `merchant_name`; a leitura traz `next_occurrence`, `monthly_equivalent` e `my_monthly_equivalent` |
| **Importação CSV** | `/{ws}/imports` | `POST /parse` (mapeia colunas + marca duplicatas), `POST /commit` (decisão por linha, idempotente enquanto o lançamento da linha existir); `GET` (as importações de quem pede, com `live_transactions` e `attachments`), `GET /{id}` (desfecho de cada linha), `POST /{id}/undo` (exclui o que o lote criou, tudo ou nada; com anexo exige `confirm_attachments`) — ADR 0036 |
...
| **Extrato de conta** | `/me/imports` | `POST /parse` (CSV de uma conta, sinal preservado, sugestão de classificação por linha), `POST /commit` (cada linha vira despesa, renda, transferência ou pagamento de fatura; as que não entram voltam em `problems`), `GET` e `GET /{id}` (os lotes de extrato da pessoa), `POST /{id}/undo` (exclui e estorna o que o extrato criou, tudo ou nada) — ADR 0037 |
