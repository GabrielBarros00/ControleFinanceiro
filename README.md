...
- **Assinaturas** — a recorrência marcada como assinatura ganha plano, teste grátis e benefícios; o quadro mostra a sua parte por mês (anual ÷ 12) e a próxima cobrança ([ADR 0039](docs/adr/0039-assinaturas.md)).
- **Estabelecimentos** — onde a despesa foi feita, com os apelidos do extrato ("IFD*MC DONALDS"): o lançamento novo se liga sozinho, e Relatórios mostra o gasto por estabelecimento ([ADR 0038](docs/adr/0038-estabelecimento.md)).
- **Financiamentos** — cronograma SAC/PRICE por mês de calendário; quitação antecipada simulada; pagar parcela vira despesa real.
- **Importação de CSV** — mapeamento de colunas, decisão por linha (importar/ignorar) e **idempotência** (reimportar não duplica). O **extrato de uma conta** entra com o sinal: cada linha vira despesa, renda, transferência ou pagamento de fatura; e dá para **desfazer** uma importação inteira ([ADR 0036](docs/adr/0036-desfazer-importacao.md), [ADR 0037](docs/adr/0037-importacao-de-extrato-de-conta.md)).
...
- **Agentes de IA (MCP)** — ChatGPT, Claude, Claude Code, Codex, Gemini CLI e Antigravity conectam à sua conta por OAuth 2.1 e consultam/registram lançamentos conversando, com as MESMAS regras e permissões do app: 59 tools (inclusive itens da nota, extrato, histórico, estabelecimentos, assinaturas e desfazer), nomes resolvidos no servidor, idempotência por chave e exclusão em massa só com prévia confirmada. Configurações › Integrações com IA mostra o endereço, os guias por cliente e as conexões ([ADR 0035](docs/adr/0035-integracao-com-agentes-de-ia-mcp.md), [docs/mcp](docs/mcp/README.md)).
