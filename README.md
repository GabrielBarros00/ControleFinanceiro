...
- **Agentes de IA (MCP)** — ChatGPT, Claude, Claude Code, Codex, Gemini CLI e Antigravity conectam à sua conta por OAuth 2.1 e consultam/registram lançamentos conversando, com as MESMAS regras e permissões do app: 57 tools (inclusive itens da nota, extrato, histórico e desfazer), nomes resolvidos no servidor, idempotência por chave e exclusão em massa só com prévia confirmada. Configurações › Integrações com IA mostra o endereço, os guias por cliente e as conexões ([ADR 0035](docs/adr/0035-integracao-com-agentes-de-ia-mcp.md), [docs/mcp](docs/mcp/README.md)).
