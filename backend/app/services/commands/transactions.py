"""Comandos de escrita de LANÇAMENTOS — a camada de aplicação que o REST e o MCP compartilham.

Até o ADR 0035 este código morava dentro das rotas de `api/routes/transactions.py`,
cada handler terminando no seu `commit`. O servidor MCP precisa das MESMAS regras
(fatura derivada no servidor, divisão em centavos, parcelamento, conversão de
moeda, máquina de estados, trava de despesa paga) — e precisa gravar a chave de
idempotência na MESMA transação da despesa. Com o commit dentro do handler, isso
era impossível sem duplicar a regra.

Então o corpo foi movido para cá SEM MUDANÇA de comportamento, com uma única
diferença: **nenhum comando comita** (ADR 0010). Quem chama — a rota REST ou o
pipeline do MCP — comanda a transação e executa os efeitos pós-commit que o
comando devolve (as chaves de anexo a liberar, `free_keys`).

Os comandos continuam levantando `HTTPException` com as mensagens de sempre: são
elas que o SPA mostra, e o MCP as traduz para o envelope de erro dele.
"""
import datetime
import re
import uuid
...
from app.services.attachment_storage import keys_to_free
...
def delete_transaction(
    ...
    return {"status": "ok", "attachments_removed": int(quantos_anexos)}, liberar
