...
class ReportService:
    @staticmethod
    def get_summary(db: Session, workspace_id: int, target_month: date) -> Dict[str, Any]:
    ...
    @staticmethod
    def get_last_6_months(db: Session, workspace_id: int) -> List[Dict[str, Any]]:
        results = []
        first_of_month = date.today().replace(day=1)
        for i in range(5, -1, -1):
            d = add_months(first_of_month, -i)  # mês de calendário, não days=30
            summary = ReportService.get_summary(db, workspace_id, d)
            results.append({
                "name": d.strftime("%b"),
